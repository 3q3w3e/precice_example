...
#$ -pe mpi_20 80
#$ -j y
#$ -o /datanode/hilbert/su2/fsi/turek_hron2_lastlastlast/turek_hron2_dt0001_medium/turek_hron2_dt0002_fine_re_re/qsub_restart.log
#$ -V
...
echo "[qsub] prepare_restart RESTART (config_fluid_restart.cfg / precice-config.xml 재생성)"
python3 "$CASE/prepare_restart.py" || { echo "[qsub] prepare_restart FAILED"; exit 1; }
...
FLUID_HOST_SPEC="node12:18,node14:20,node15:20,node16:20" FLUID_RESTART=1 MPIRUN_EXTRA="--bind-to core" CONFIG_FILE=config_fluid_restart.cfg "$CASE/fluid/run.sh" >/dev/null 2>&1 &
FPID=$!
sleep 3
SOLID_RESTART=1 SOLID_NODE=node12 SOLID_OMP_THREADS=2 SOLID_CPU_SET=18,19 "$CASE/solid/run.sh" >/dev/null 2>&1 &
SPID=$!
wait "$FPID"; FRC=$?
wait "$SPID"; SRC=$?
