...
#      SOLID_CPU_SET=12,14 ./run.sh      # solid OMP 가 사용할 OS CPU 번호
#
...
# └──────────────────────────────────────────────────────────┘
SOLID_NODE="${SOLID_NODE:-node16}"                  # 계산 노드 (유휴 추천: node16/17/20, 20코어)
SOLID_OMP_THREADS="${SOLID_OMP_THREADS=2}"      # auto = 노드 물리코어수 자동 산출
SOLID_CPU_SET="${SOLID_CPU_SET:-12,14}"               # taskset OS CPU 번호. fluid socket1 홀수와 같은 socket1 의 남은 두 코어 (17,19), socket0 의 hron1 fluid 와 충돌 회피
