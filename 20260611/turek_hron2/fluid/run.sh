...
# └──────────────────────────────────────────────────────────┘
FLUID_NODES="${FLUID_NODES:-node16}"           # 사용할 계산 노드 (공백 구분)
FLUID_SLOTS_PER_NODE="${FLUID_SLOTS_PER_NODE=6}"  # 노드당 MPI rank 수. auto = 노드 물리코어수
FLUID_CPU_LIST="${FLUID_CPU_LIST:-0,1,2,3,4,5}"  # OpenMPI --cpu-list 는 hwloc 논리번호 (OS 물리 아님). 이 값(=logical 10..17) → OS physical 1,3,5,7,9,11,13,15 = socket1 홀수 8개. solid SOLID_CPU_SET(=OS physical 12,14) 와 겹치지 않음. 매핑 확인: lstopo --of console
# ────────────────────────────────────────────────────────────
