...
#      SOLID_CPU_SET=13,15 ./run.sh      # solid OMP 가 사용할 OS CPU 번호
#
...
# └──────────────────────────────────────────────────────────┘
SOLID_NODE="${SOLID_NODE:-node10}"                  # 계산 노드 (유휴 추천: node16/17/20, 20코어)
SOLID_OMP_THREADS="${SOLID_OMP_THREADS=2}"      # auto = 노드 물리코어수 자동 산출
SOLID_CPU_SET="${SOLID_CPU_SET:-13,15}"               # taskset OS CPU 번호. node10 의 hwloc 14,15 = OS 13,15 (socket1 의 마지막 2 코어). fluid 가 node10 의 hwloc 0..13 (= 첫 14 코어) 차지하므로 충돌 없음
