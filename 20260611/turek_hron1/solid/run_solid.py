...
E_MOD, NU, RHO = 1.4e6, 0.40, 1.0e3      # solid.py 와 동일
...
def write_window_med(result, step, t, save_every=20):                       # ← t 인자 추가
...
# 일부 coupling 설정은 첫 교환 전에 초기 데이터를 요구한다 (이 config 는 보통 불필요)
if participant.requires_initial_data():
    participant.write_data(MESH_NAME, WRITE_DATA, vertex_ids,
                           np.zeros((n_iface, DIM), dtype=np.float64))

participant.initialize()  # max_timestep을 반환함

# ============ implicit coupling 루프 ============
result,     t     = None, 0.0     # 현재까지 수렴된 상태 / 시각
result_ckp, t_ckp = None, 0.0     # checkpoint = 현재 window 시작점
step = 0                          # 완료된 window 개수
...
try:
    while participant.is_coupling_ongoing():
        ...
        if result_ckp is None and float(np.linalg.norm(forces)) <= ZERO_FORCE_TOL:
        ...
        if participant.requires_reading_checkpoint():
            ...
            if participant.is_time_window_complete():
                ...
                if trial is not None:
                    write_window_med(trial, step, t, save_every=20)
