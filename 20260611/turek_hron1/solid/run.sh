...
# └──────────────────────────────────────────────────────────┘
SOLID_NODE="${SOLID_NODE:-node09}"                  # 계산 노드 (유휴 추천: node16/17/20, 20코어)
SOLID_OMP_THREADS="${SOLID_OMP_THREADS=2}"      # auto = 노드 물리코어수 자동 산출
SOLID_CPU_SET="${SOLID_CPU_SET:-12,14}"               # taskset OS CPU 번호. node09 socket0 의 남은 코어 (hwloc 6,7 = OS 12,14). fluid 가 hwloc 0..5 차지하므로 겹침 없음
