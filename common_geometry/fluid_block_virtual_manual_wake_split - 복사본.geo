...
xCs = 0.45; // upstream cross-section station
...
// 1.4 Structured mesh size controls.
// Smaller scale values are finer.  These two knobs control the mostly
// streamwise and mostly wall-normal curve families independently.
x_cell_size_scale = 1.0;
y_cell_size_scale = 1.0;

// Manual counts for directions that are not purely x or y, or are locked by
// topology.  n_radial also sets the near-wake x count to keep transfinite
// blocks structured.
n_cgrid_arc_manual = 7;
n_radial_manual    = 5;
ny_tip_half_manual = 4;

// Downstream wake cells grow smoothly from xWd to L.  0 means uniform;
// larger values coarsen more toward the outlet without a sharp jump at xWd.
wake_growth_strength = 1;
...
// 6.1 Derived counts.  Do not edit these directly.
// Counts are node counts, computed from nominal x/y target cell sizes and
// then locked to the dependencies required by the transfinite topology.
target_x_cell_size = 0.0125 * x_cell_size_scale;
target_y_cell_size = 0.0125 * y_cell_size_scale;

nx_inlet = Max(3, Round(x225v / target_x_cell_size) + 1);
nx_cgrid_arc = Max(4, n_cgrid_arc_manual);
nx_cgrid_mid = 2*nx_cgrid_arc - 1;

n_radial = Max(3, n_radial_manual);
nx_wake_near = n_radial;

ny_lower = Max(3, Round(y315v / target_y_cell_size) + 1);
ny_upper = Max(3, Round((H - y45v) / target_y_cell_size) + 1);
ny_tip_half = Max(2, ny_tip_half_manual);
ny_beam_gap = 2*ny_tip_half - 1;

nx_xcs_inner_left = Max(3, Round((xCs - xRi) / target_x_cell_size) + 1);
nx_xcs_inner_right = Max(3, Round((xWt - xCs) / target_x_cell_size) + 1);
nx_xcs_outer_left = nx_xcs_inner_left + ny_beam_gap + n_radial - 2;

nx_wake_split = Max(3, Round((xWd - xWt) / target_x_cell_size) + 1);
nx_tail_left = Max(3, Round((xCt - xWd) / target_x_cell_size) + 1);
nx_tail_right = Max(3, Round((L - xCt) / target_x_cell_size) + 1);

wake_grow_left = 1 + wake_growth_strength / Max(1, nx_tail_left - 1);
wake_grow_right = 1 + wake_growth_strength / Max(1, nx_tail_right - 1);
